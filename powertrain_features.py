...
from powertrain_model_orig import PowertrainModel
...
class BatteryFeature(PowertrainComponent):
    def initialize(self, kwargs):
        super().initialize(kwargs)
        # self.parameters.declare('name', types=str)
        self.parameters.declare('level', default=1.0, lower=0.0, upper=1.0)
...
class MotorFeature(PowertrainComponent):
    def initialize(self, kwargs):
        super().initialize(kwargs)
        # self.parameters.declare('name', types=str)
        self.parameters.declare('num_rotors', types=int, default=1)
