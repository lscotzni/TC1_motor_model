...
class MaxTorqueImplicitModelTest(Model):
    ...
    def define(self):
        ...
        quartic_discriminant = self.solve_quartic_discriminant(a,b,c,d,e)
        T_lim_residual = csdl.log10(quartic_discriminant**2 + 1) / 1e6

        T_lim_residual = self.register_output(
            'T_lim_residual',
            self.solve_quartic_discriminant(a,b,c,d,e) / 1e16
            # T_lim_residual
        )
