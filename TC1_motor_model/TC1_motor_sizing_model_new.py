...
class TC1MotorSizingModel(Model):
    ...
    def define(self):
        ...
        # pole_pitch = self.register_output('pole_pitch', np.pi*D_i/(2*p))
        # tooth_pitch = self.register_output('tooth_pitch', np.pi*D_i/Z)
        ...
        # l_ef = self.register_output('l_ef', L + 2*air_gap_depth) # final effective length of motor
        # rotor_radius = self.register_output('rotor_radius', D_i - 2*air_gap_depth) # D2 in MATLAB code
        air_gap_depth = 0.4*line_load*pole_pitch/(0.9e-6*B_air_gap_max)
        ...
        # Acu = self.register_output('Acu',I_w/(a*J*conductors_per_slot) * 1e-6)
        ...
        # tooth_width = self.register_output('tooth_width', tooth_pitch * B_air_gap_max / (kfe * Bt)) # STATOR TOOTH WIDTH
        tooth_width = tooth_pitch * B_air_gap_max / (kfe * Bt)

        B_ys = 1.35 # FLUX DENSITY IN STATOR YOKE
        # h_ys = self.register_output('height_yoke_stator',(pole_pitch*alpha_B*B_air_gap_max) / (2*kfe*B_ys)) # HEIGHT OF YOKE IN STATOR
        ...
        # b_sb = self.register_output('slot_bottom_width', theta_ssi*np.pi*D_i/360) # WIDTH OF BOTTOM OF SLOT
        # h_slot = self.register_output('slot_height', (outer_stator_radius - D_i)/2 - h_ys) # HEIGHT OF SLOT
        ...
        # b_s1 = self.register_output('slot_width_inner', (np.pi*(D_i+2*(h_os+h_k)))/36 - tooth_width) # RADIALLY INNER WIDTH OF SLOT
        ...
        # Tau_y = self.register_output('Tau_y', np.pi*(D_i+h_slot) / (2*p))
        ...
        motor_variables[7]  = tooth_width 
        motor_variables[8]  = h_ys 
        motor_variables[9]  = b_sb 
        motor_variables[10] = h_slot 
        motor_variables[11] = b_s1 
        motor_variables[12] = Tau_y 
        motor_variables[13] = L_j1 
        motor_variables[14] = Kdp1 
        motor_variables[15] = bm 
        motor_variables[16] = Am_r 
        motor_variables[17] = phi_r 
        motor_variables[18] = lambda_m 
        motor_variables[19] = Kf 
        motor_variables[20] = K_phi 
        motor_variables[21] = K_theta 
        motor_variables[22] = A_f2