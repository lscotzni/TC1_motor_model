...
class TC1MotorAnalysisModel(Model):
    ...
    # ========================= IMPLICIT T_EM MODEL =========================
        ...
        if False:
            ...
            # ========================= IMPLICIT T_EM MODEL =========================
            model=Model()
            T_em = model.declare_variable('T_em', val=500, shape=(num_nodes,)) # STATE of implicit model
            ...
            PsiF_expanded = csdl.expand(phi_air, (num_nodes,))

            Iq_fw = T_em / (1.5*p * (PsiF_expanded + (L_d_expanded-L_q_expanded)*Id_fw)) # CHECK SIZE OF COMPUTATIONS HERE
